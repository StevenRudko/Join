const DB_URL =
  "https://join-95521-default-rtdb.europe-west1.firebasedatabase.app/";
...
function updateGreetingElements(userData) {
  ...
  const greetingTimeMobileElement = document.getElementById(
    "greeting-time-mobile"
  );
  const greetingUserNameMobileElement = document.getElementById(
...
function extractPriority(priorityText) {
  ...
  const match = validPriorities.find((priority) =>
    priorityText.toLowerCase().startsWith(priority)
  );
